    ...
    return this.database.transaction(async (executor) => {
      ...
      for (const candidate of ranked) {
        const reservation = await executor.query<{
          ...
          [randomUUID(), candidate.driverId, trip.id, OFFER_TTL_SECONDS],
        );
        ...
        const offer = await executor.query<OfferRow>(
          ...
          [
            ...
            reservation.rows[0].id,
            candidate.driverId,
            await nextAttempt(executor, trip.id),
        ...
        const response = {
          ...
          tripState,
          tripVersion,
          offer: toOfferResponse(offer.rows[0] as OfferRow),
        };
        await persistMatchingReceipt(executor, input, response, trip.id);
        return response;
      }
