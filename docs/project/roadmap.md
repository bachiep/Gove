...
Current milestone: **M2 Ride Request and Pricing — implemented and tested locally.** The API, concurrency suite, migration replay, and authenticated customer browser smoke flow are verified locally. CI has been defined but is not considered verified until the workflow succeeds on GitHub.
